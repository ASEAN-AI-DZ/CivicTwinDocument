...
Hướng dẫn cài đặt và chạy CivicTwin AI **KHÔNG sử dụng Docker** (bare metal).
...
CREATE USER civictwin_user WITH PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE civictwin TO civictwin_user;
...
Create service file `/etc/systemd/system/civictwin-core.service`:

```ini
[Unit]
Description=civictwin Core API
After=network.target postgresql.service redis.service

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/civictwin/modules/CoreAPI
...
sudo systemctl enable civictwin-core
sudo systemctl start civictwin-core
```
...
**File: `/etc/nginx/sites-available/civictwin`**
...
server {
    ...
    # Static files
    location /storage/ {
        alias /var/www/civictwin/modules/CoreAPI/storage/app/public/;
    }
}
```
...
```bash
sudo ln -s /etc/nginx/sites-available/civictwin /etc/nginx/sites-enabled/
sudo nginx -t
